...
void add_torus( struct matrix * edges,
	...
	struct matrix *newTorus = generate_torus(cx, cy, cz, r1, r2, step);
	int ctr;
	for (ctr = 0; ctr < newTorus->lastcol; ctr++) {
		double x = newTorus->m[0][ctr];
		double y = newTorus->m[1][ctr];
		double z = newTorus->m[2][ctr];
...
struct matrix * generate_torus( double cx, double cy, double cz,
				double r1, double r2, double step ) {
	struct matrix *newTorus = new_matrix(4, 4);
	...
	for(ctr1 = 0; ctr1 <= num; ctr1++) {
		...
		for(ctr2 = 0; ctr2 <= num; ctr2++) {
			...
			double B = ((M_PI * 2)/num) * ctr2;

			double x = cos(B) * (r1 * cos(A) + r2) + cx;
			double y = r1 * sin(A) + cy;
			double z = -sin(B) * (r1 * cos(A) + r2) + cz;
			add_point(newTorus, x, y, z);
		}
	}
	return newTorus;
}
